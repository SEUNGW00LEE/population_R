...
tota_all = total_all %>% gather(key = years, value = total_population)


total_population <-
  ggplot(total_all, aes(x=area, y=population, fill = area)) +
  geom_col(show.legend=FALSE)+
  geom_text(aes(x=area, y=population, label=population), vjust = -0.7, family = "AppleSDGothicNeo-SemiBold")+
  scale_color_brewer(palette = "Set3")+
  theme_minimal()+
  theme(text = element_text(size = 11 ,   family = "AppleSDGothicNeo-SemiBold", face = "bold"),
        plot.title = element_text(hjust = 0.5,size=20, family = "AppleSDGothicNeo-SemiBold", face='bold', color = "darkblue"))+
